  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="$emit('close')">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800">Create Claim Right</h2>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <div class="text-sm text-gray-500 mt-1">Prepaid expenses, subscriptions</div>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <div class="text-sm text-gray-500 mt-1">Deferred revenue, advance payments</div>
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            type="text"
            required
            placeholder="e.g., Annual subscription to SaaS platform"
            class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-gray-900 focus:border-transparent"
          />
        </div>

        <!-- Ledger Entry ID (Optional) -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">
            Ledger Entry ID (Optional)
          </label>
          <input
            v-model.number="form.ledger_entry_id"
            type="number"
            placeholder="Link to existing transaction"
            class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-gray-900 focus:border-transparent"
          />
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.ledger_entry_id) {
      requestData.ledger_entry_id = form.ledger_entry_id
    }

    await api.createClaimRight(requestData)
    emit('created')
  } catch (err) {
