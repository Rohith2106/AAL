    ...
    <div class="bg-white/80 backdrop-blur-xl border border-white/40 shadow-2xl rounded-3xl p-8 w-full max-w-md">
      ...
      <div class="mt-6 text-center">
        <p class="text-sm text-gray-600">
          ...
          <router-link to="/login" class="text-gray-900 font-semibold hover:underline">Sign in</router-link>
        </p>
      </div>
    </div>
  </div>
...
const handleSignup = async () => {
  ...
  try {
    const response = await api.signup(email.value, password.value, companyName.value || undefined)
